...
void PlayerManager::SetUp() {
	...
	for (auto& player : players) {
		...
		groundedBody.SetPosition(
			newBody.Position() -
			Math::Vec2F(grounded_collider_dimension_.X * 0.5f, grounded_collider_pos_y_));
	...
	for (auto& player : players) {
		...
		case Math::ShapeType::Rectangle:
			collider.rectangleShape = Math::RectangleF(
				position + collider.rectangleShape.MinBound(), position + collider.rectangleShape.MaxBound() -
...
void PlayerManager::Jump(int playerIdx) {
	...
	// std::cout << players[playerIdx].is_grounded << std::endl;
	std::cout << trigger_nbrs_[playerIdx] << std::endl;
...
void PlayerManager::Move(bool rightDirection, int playerIdx) {
	...
	body.SetVelocity(Math::Vec2F::Lerp(
		body.Velocity(), { targetVelocityX, body.Velocity().Y }, acceleration_time_));
}
