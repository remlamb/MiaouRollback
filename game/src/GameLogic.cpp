...
namespace game {
	...
	void GameLogic::OnFrameConfirmationReceived(
		...
		if (rollback_manager->last_remote_input_frame() <
			...
			rollback_manager->SetRemotePlayerInput(frame_inputs,
				other_client_id);
		}
	...
	void GameLogic::OnInputReceived(const ExitGames::Common::Hashtable& content)
	{
		...
		for (int i = 0; i < inputs_count; i++) {
			...
			//std::cout << static_cast<int>(frame_input.input) << " , " << frame_input.frame_nbr << std::endl;
		}
		...
		const int other_client_id = client_player_nbr == 0 ? 1 : 0;
		rollback_manager->SetRemotePlayerInput(remote_frame_inputs,
			other_client_id);
	...
	void GameLogic::ManageInput() noexcept {
		...
		event_data.put(static_cast<nByte>(EventCode::kInput),
			last_inputs.data(),
	...
	void GameLogic::UpdateGameplay() noexcept
	{
		...
		for (int i = 0; i < 2; i++)
		{
			...
			if (player_manager.players[i].input & static_cast<std::uint8_t>(Input::kLeft)) {
				player_manager.Move(false, i);
			}
			else if (player_manager.players[i].input & static_cast<std::uint8_t>(Input::kRight)) {
